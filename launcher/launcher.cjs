...
function createWindow() {
  ...
  const html = `<!doctype html><html lang="ko"><head><meta charset="utf-8"><style>
    *{box-sizing:border-box} body{margin:0;background:#f5faf8;font-family:"Segoe UI","Malgun Gothic",sans-serif;color:#153449;display:flex;align-items:center;justify-content:center;height:100vh}
    .card{width:520px;background:#fff;border:1px solid #d8e6e1;border-radius:22px;padding:30px 34px;box-shadow:0 18px 55px rgba(28,74,66,.13)}
    .head{display:flex;align-items:center;gap:16px}.logo{width:54px;height:54px;border-radius:17px;background:#dff3ee;display:flex;align-items:center;justify-content:center;color:#159981;font-weight:800;font-size:29px}
    h1{font-size:23px;margin:0 0 5px;font-weight:800}.sub{font-size:12px;color:#7e959e;letter-spacing:.4px}
    .status{margin-top:28px;font-size:16px;font-weight:700}.detail{margin-top:8px;font-size:13px;color:#718a94;min-height:20px}
    .bar{margin-top:22px;height:9px;border-radius:999px;background:#e6efec;overflow:hidden}.fill{height:100%;width:8%;background:#62c3b2;border-radius:999px;transition:width .2s ease}
    .foot{margin-top:18px;display:flex;justify-content:space-between;font-size:12px;color:#8aa0a7}.pct{font-weight:700;color:#4f8178}
  </style></head><body><div class="card"><div class="head"><div class="logo">U</div><div><h1>UEP 실행 준비</h1><div class="sub">UNHO EDUCATION PLATFORM</div></div></div><div id="status" class="status">업데이트를 확인하고 있습니다.</div><div id="detail" class="detail">잠시만 기다려 주세요.</div><div class="bar"><div id="fill" class="fill"></div></div><div class="foot"><span id="version">현재 버전 확인 중</span><span id="pct" class="pct"></span></div></div><script>
    window.setUEPStatus=(s,d,p,v)=>{document.getElementById('status').textContent=s||'';document.getElementById('detail').textContent=d||'';if(Number.isFinite(p))document.getElementById('fill').style.width=Math.max(3,Math.min(100,p))+'%';document.getElementById('pct').textContent=Number.isFinite(p)?Math.round(p)+'%':'';if(v)document.getElementById('version').textContent=v;};
  </script></body></html>`;
...
function requestJson(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { headers: { 'User-Agent': 'UEP-Launcher/1.2' }, timeout: 8000 }, r => {
    ...
    req.on('timeout', () => req.destroy(new Error('정책 서버 연결 시간 초과'))); req.on('error', reject);
  });
}
function download(url, dest, onProgress) {
  return new Promise((resolve,reject)=>{
    const go=u=>{const req=https.get(u,{headers:{'User-Agent':'UEP-Launcher/1.2'},timeout:45000},r=>{
      ...
      const total=Number(r.headers['content-length']||0); let got=0; const f=fs.createWriteStream(dest);
      r.on('data',chunk=>{got+=chunk.length;if(total>0)onProgress?.(got,total);}); r.pipe(f); f.on('finish',()=>f.close(resolve)); f.on('error',reject);
    });req.on('timeout',()=>req.destroy(new Error('업데이트 다운로드 시간 초과')));req.on('error',reject);}; go(url);
  });
}
...
async function installUpdate(policy,cur){
  const tmp=path.join(app.getPath('temp'),`UEP-update-${Date.now()}.zip`); const stage=path.join(app.getPath('temp'),`UEP-stage-${Date.now()}`);
  ...
  status('업데이트 파일을 검증하고 있습니다.','무결성을 확인하는 중입니다.',74,`최신 ${policy.latestVersion}`);
...
app.whenReady().then(async()=>{
  createWindow(); const cur=localVersion(); status('업데이트를 확인하고 있습니다.','최신 배포 정보를 확인하는 중입니다.',6,`현재 버전 ${cur}`);
  let policy; let online=true;
  ...
  try{
    const needsUpdate=!!(policy.updateEnabled&&policy.latestVersion&&cmp(policy.latestVersion,cur)>0); const mustUpdate=!!(policy.forceUpdate&&(isBlocked(policy,cur)||(policy.minimumAllowedVersion&&cmp(cur,policy.minimumAllowedVersion)<0)));
    if(needsUpdate){if(!online&&mustUpdate)throw new Error(policy.message||'필수 업데이트가 필요하지만 업데이트 서버에 연결할 수 없습니다.');if(!policy.downloadUrl)throw new Error('업데이트 주소가 설정되지 않았습니다.');await installUpdate(policy,cur);status('업데이트가 완료되었습니다.','최신 UEP를 실행합니다.',100,`UEP ${policy.latestVersion}`);}
    else{assertAllowedWithoutUpdate(policy,cur);status('최신 버전입니다.','UEP를 실행합니다.',100,`UEP ${cur}`);}
    setTimeout(()=>{launch();app.quit();},800);
