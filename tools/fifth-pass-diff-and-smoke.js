...
const smokeMap={tasksMarkup:['dashboard','업무/할일 표시'],dormProgramIsHolisticTarget:['dorm','기숙/프로그램 대상 판정'],nightMatrixStatus:['attendance','야자 상태표'],admissionCutLimit:['admissions','입시 컷'],counselRecordLabel:['records','상담 기록 라벨'],recordAreaPanel:['records','생활기록부 영역'],dateDistanceDays:['records','날짜거리'],careerDateEditor:['records','진로 날짜 편집'],recordEvidenceSignals:['records','기록 근거 신호'],stripProgramDatesFromRecordText:['records','프로그램 날짜 제거'],programTopTabs:['programs','프로그램 상단탭'],formatTaskCompletedAt:['dashboard','업무 완료시각'],fixedTeacherTimetableReference:['timetable','교사시간표'],homeroomWeekMarkup:['timetable','담임 주간표'],workItemReadByCurrentUser:['work','업무 읽음'],canRevealStudentSensitiveInfo:['students','민감정보 권한'],signalRowsForStudent:['students','학생 신호'],openStudentTimetableDrawer:['students','학생시간표 팝업'],sortUniversitiesByPriority:['admissions','대학 정렬'],studentStatsRankCard:['scores','학생 순위 카드'],copyProgramIndividualNotices:['programs','개별 안내'],copyProgramFamilyLetter:['programs','가정통신문']};
...
report.push('','## Decision rule','- This pass is fail-closed: incomplete duplicate or smoke coverage fails CI.','- SAFE_DELETE_AFTER_SMOKE count is derived from the immediately preceding fourth-pass classification; no historical hard-coded count is allowed.','- FLATTEN rows are never deleted mechanically; preserve wrapper-added behavior in one canonical implementation.','- Smoke candidates are not deleted in this pass.');
...
console.log(`FIFTH PASS COMPLETE names=${diffNames.length}/${expectedNames.length} diffs=${diffs.length} smoke=${smoke.length}/${candidates.length} manual=${manual.length}`);