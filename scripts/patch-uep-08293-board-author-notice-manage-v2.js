...
function replaceFunction(text,name,newSource){
  const rx=new RegExp('(?:async\\s+)?function\\s+'+name+'\\s*\\(');const m=rx.exec(text);must(m,name+' not found');const start=m.index;const brace=text.indexOf('{',m.index);must(brace>=0,name+' brace not found');
  let depth=0,end=-1,quote='',esc=false;
  for(let i=brace;i<text.length;i++){const ch=text[i];if(quote){if(esc){esc=false;continue;}if(ch==='\\'){esc=true;continue;}if(ch===quote)quote='';continue;}if(ch==='"'||ch==="'"||ch==='`'){quote=ch;continue;}if(ch==='{')depth++;else if(ch==='}'){depth--;if(depth===0){end=i+1;break;}}}
  must(end>0,name+' end not found');return text.slice(0,start)+newSource+text.slice(end);
}
let renderer=read(rendererPath),main=read(mainPath),preload=read(preloadPath),index=read(indexPath);const pkg=JSON.parse(read(packagePath));
must(pkg.version==='0.82.92','baseline package version mismatch: '+pkg.version);pkg.version='0.82.93';
...
const oldAppend="await boardAppend08291(ctx.token,noticeSheet08291(t),row);await boardAudit08291(ctx,'공지','SUCCESS',id+' / '+t.label,id);return {ok:true,id,target:t.label,sheet:noticeSheet08291(t)};";
const newAppend="const appended=await boardAppend08291(ctx.token,noticeSheet08291(t),row);const verified=await boardVerifyAppend08293(ctx.token,appended,id,ctx.displayAuthor);if(!verified.ok)throw boardWriteError08291('Board DB 저장 후 재확인에 실패했습니다: '+verified.reason,'UEP_BOARD_NOTICE_VERIFY');await boardAudit08291(ctx,'공지','SUCCESS',id+' / '+t.label,id);return {ok:true,id,target:t.label,sheet:noticeSheet08291(t),verified:true,range:verified.range,author:ctx.displayAuthor};";
main=replaceOnce(main,oldAppend,newAppend,'notice readback');
const oldIpc='ipcMain.handle("uep:boardDbStatus", async () => {try{return await boardDbStatus08291();}catch(error){return {ok:false,code:error?.code||\'UEP_BOARD_STATUS_ERROR\',message:error?.message||String(error),boards:[]};}});';
...
main=replaceOnce(main,oldIpc,newIpc,'notice ipc');
preload=replaceOnce(preload,'boardDbStatus: () => ipcRenderer.invoke("uep:boardDbStatus"),','boardDbStatus: () => ipcRenderer.invoke("uep:boardDbStatus"),\n  boardNoticeList: () => ipcRenderer.invoke("uep:boardNoticeList"),\n  boardNoticeDelete: (payload) => ipcRenderer.invoke("uep:boardNoticeDelete", payload||{}),','preload notice');

const rendererHelpers=[
'// __UEP_08293_BOARD_AUTHOR_NOTICE_MANAGE_RENDERER__',
"function boardCurrentAuthor08293(){return String((typeof currentLoginTeacherName==='function'?currentLoginTeacherName():'')||(typeof currentUserProfile==='function'?currentUserProfile()?.name:'')||state?.auth?.user?.name||'').trim();}",
'async function refreshBoardNoticeManager08293(){',
"  const host=document.getElementById('boardNoticeManager08293');if(!host||!window.schoolBoard?.boardNoticeList)return;host.innerHTML='<div class=\"board-notice-empty\">현재 공지를 불러오는 중입니다.</div>';",
...
"    host.innerHTML=rows.map(function(n){const del=n.canDelete?'<button type=\"button\" class=\"btn secondary danger\" data-board-notice-delete=\"'+escapeHtml(n.noticeId||'')+'\" data-board-notice-sheet=\"'+escapeHtml(n.sheet||'')+'\">삭제</button>':'';return '<article class=\"board-notice-manage-card\"><div><small>'+escapeHtml(n.target||'')+' · '+escapeHtml(n.importance||'일반')+'</small><strong>'+escapeHtml(n.title||'')+'</strong><p>'+escapeHtml(n.message||'')+'</p><em>'+escapeHtml(n.author||'-')+' · '+escapeHtml(n.startDate||'')+' ~ '+escapeHtml(n.endDate||'')+'</em></div>'+del+'</article>';}).join('');",
"    host.querySelectorAll('[data-board-notice-delete]').forEach(function(btn){btn.addEventListener('click',function(){deleteBoardNotice08293(btn.dataset.boardNoticeSheet||'',btn.dataset.boardNoticeDelete||'');});});",
