...
// requestAnimationFrame 첫 줄에서 예외가 나면 문이과 교차오류/과목별 신청현황 전체가 클릭 불가가 된다.
...
// google-data가 officialAttendance.date를 YYYY-MM-DD로 이미 정규화하므로 재파싱/offset 금지.
...
// 3) 직접 학교업무 공지: 일반공지도 확인 대상. 관리자 0/9와 담임 확인 버튼의 의미를 일치시킨다.
...
// 기존 0.81.61에 이미 등록된 일반공지(confirmRequired=false)도 직접 학교업무 공지면 확인 가능하게 한다.
...
// 직접공지 확인 저장 후 공용 캐시를 강제 갱신하여 관리자 집계와 즉시 연결.
...
assert(!g.includes("$('[data-curriculum-workspace]').forEach"),'singular curriculum selector remains');
assert(!g.includes("$('[data-cross-student]').forEach"),'singular cross-student selector remains');
assert(!g.includes("$('[data-curriculum-subject]').forEach"),'singular curriculum-subject selector remains');
