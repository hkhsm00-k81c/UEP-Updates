...
async function runSelectionSheetWriteProbe08195(){
 ...
 const probe='UEP_WRITE_TEST_'+Date.now();
 await updateSheetValues(token,id,range,[[probe]]);
 const chk=(await readSheetBatch(token,id,[range]))?.[0]?.values||[];
 const actual=String(chk?.[0]?.[0]??'');
 if(actual!==probe)throw new Error('52 A4 readback mismatch: '+actual);
 const marker='UEP_WRITE_OK | '+new Date().toISOString();
 await updateSheetValues(token,id,range,[[marker]]);
 const finalChk=(await readSheetBatch(token,id,[range]))?.[0]?.values||[];
 const finalActual=String(finalChk?.[0]?.[0]??'');
 if(finalActual!==marker)throw new Error('52 A4 success-marker readback mismatch: '+finalActual);
 return {ok:true,marker,writeVerified:true,markerVerified:true,checkedAt:new Date().toISOString()};
}
...
if(!g.includes('UEP_08195_SHEET_WRITE_PROBE_RENDERER_START'))g+=String.raw`
/* UEP_08195_SHEET_WRITE_PROBE_RENDERER_START */
(function(){if(typeof window==='undefined'||window.__UEP08195SheetWriteProbeInstalled)return;window.__UEP08195SheetWriteProbeInstalled=true;window.runSelectionSheetWriteProbe08195=async()=>{try{const r=await window.schoolBoard?.sheetWriteProbe08195?.();console.log('[08195-probe] 52 A4 write/read/marker',r);window.__UEP08195SheetWriteProbeResult=r;return r}catch(e){const r={ok:false,reason:e?.message||String(e)};window.__UEP08195SheetWriteProbeResult=r;console.error('[08195-probe]',r);return r}};const boot=()=>setTimeout(()=>window.runSelectionSheetWriteProbe08195(),2500);if(document.readyState==='complete')boot();else window.addEventListener('load',boot,{once:true});})();
...
console.log('installed isolated 52 A4 write/read probe with persistent success marker');
