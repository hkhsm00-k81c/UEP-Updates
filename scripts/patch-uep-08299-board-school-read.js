...
function replaceOnce(text,from,to,label){const i=text.indexOf(from);must(i>=0,label+' source not found');must(text.indexOf(from,i+from.length)<0,label+' source not unique');return text.slice(0,i)+to+text.slice(i+from.length);}
...
const oldRead=`async function boardReadRange08291(token,a1){
...
const newRead=`// __UEP_08299_BOARD_SCHOOL_READ__
...
main=replaceOnce(main,oldRead,newRead,'boardReadRange08291');

const oldContext=`async function boardDbReadContext08294(ui={}){
...
const newContext=`async function boardDbReadContext08294(ui={}){
  let status=null;try{status=await schoolReadSessionStatus({verify:true});}catch{}
  const verified=Boolean(status?.authenticated&&status?.user);
  const uiUser=ui&&typeof ui==='object'?ui:{};
  ...
  const user=verified?{
    ...
    email:boardSafeText08291(uiUser.email,120),
    grade:boardSafeText08291(uiUser.grade,10),
    homeroom:boardSafeText08291(uiUser.homeroom||uiUser.classNo,10),
    role:boardSafeText08291(uiUser.role,120),
    isAdmin:false
  };
  if(verified){
...
main=replaceOnce(main,oldContext,newContext,'boardDbReadContext08294');
...
main=replaceOnce(main,primeNeedle,primeInsert,'boardDbDetail08296 prime');
