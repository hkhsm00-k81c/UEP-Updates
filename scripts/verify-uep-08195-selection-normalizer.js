...
A(g.includes('UEP_08195_PRIORTERM_FIX_START'),'prior-term flow fix missing');
A(g.includes("for(const raw of [term,linked])"),'prior-term parser does not inspect both term fields');
A(g.includes("if(p.length>1)return[p[0],p[p.length-1]]"),'arrow-based term flow parsing missing');
