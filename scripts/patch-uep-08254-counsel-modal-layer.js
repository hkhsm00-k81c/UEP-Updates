...
g=g.replace(/const UEP_08221_RELEASE_NOTES=\[[\s\S]*?\];/,"const UEP_08221_RELEASE_NOTES=['0.82.54 · 대학전형 카드 정렬·구분 개선','교과 → 종합 → 논술 → 개인 지원자격 제한 순으로 대학 전형카드를 정렬','교과·종합·논술·지원자격 제한을 카드 색으로 구분하고 농어촌·서해5도·특성화고 등 운호고 구조적 지원불가 전형은 별도 색으로 표시','학교장추천은 학생 개인 자격제한으로 분류하지 않음','상담포인트 편집 팝업이 오늘의 대학 팝업 위에 정상 표시되도록 레이어 수정','상단 버전 표시는 실제 실행 버전을 기준으로 정상화'];");
