...
// Keep the top version pill on the actual running version. Older patches left CURRENT behind,
// which caused a normal 0.82.53 installation to show e.g. 0.82.49 -> 0.82.53.
...
// Keep the counseling editor above the Today University modal and its backdrop.
...
const counselNew="layer.className='counsel-reason-layer uep-admission-counsel-editor-layer';layer.style.position='fixed';layer.style.inset='0';layer.style.zIndex='2147483000';";
g=g.replace(counselOld,counselNew);

// 0.82.54 university-admission classification is performed from structured 53B fields first.
// School-principal recommendation is explicitly NOT treated as a student eligibility restriction.
...
const admissionHtmlOld="  const admissionHtml=admissions.length?admissions.slice(0,18).map(r=>'<article class=\"uep-uni-admission-card\"><div class=\"uep-uni-badges\">'+uep08223UniversityBadges(r)+'</div><h4>'+escapeHtml(r['전형유형']||r['대전형']||'전형')+' · '+escapeHtml(r['전형명']||'전형명 확인')+'</h4><p>'+escapeHtml(r['선발방식']||r['평가구조요약']||dashboardAdmissionMethod(r))+'</p><small><b>수능최저</b> '+escapeHtml(r['수능최저']||r['수능최저원문']||(String(r['배지사용여부']).toUpperCase()==='N'?'참고/검증중':'미적용 또는 확인 필요'))+'</small></article>').join(''):'<p>대학별 전형구조 자료를 연결 중입니다.</p>';";
must(g.includes(admissionHtmlOld),'admission card renderer anchor not found');
const admissionHtmlNew="  const admissionHtml=admissions.length?admissions.slice(0,18).map(r=>'<article class=\"uep-uni-admission-card'+uep08254AdmissionCardClass(r)+'\"><div class=\"uep-uni-badges\">'+uep08223UniversityBadges(r)+uep08254AdmissionEligibilityBadge(r)+'</div><h4>'+escapeHtml(r['전형유형']||r['대전형']||'전형')+' · '+escapeHtml(r['전형명']||'전형명 확인')+'</h4><p>'+escapeHtml(r['선발방식']||r['평가구조요약']||dashboardAdmissionMethod(r))+'</p><small><b>수능최저</b> '+escapeHtml(r['수능최저']||r['수능최저원문']||(String(r['배지사용여부']).toUpperCase()==='N'?'참고/검증중':'미적용 또는 확인 필요'))+'</small></article>').join(''):'<p>대학별 전형구조 자료를 연결 중입니다.</p>';";
g=g.replace(admissionHtmlOld,admissionHtmlNew);

// Extend the existing university-detail style block rather than adding another runtime style layer.
