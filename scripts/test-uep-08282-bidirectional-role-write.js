...
must(g,'canWriteDuty?[`'.replace('[`','`').slice(0,0),'noop');
...
must(g,'window.schoolBoard.saveNightSupervisor','night renderer -> preload bridge');
must(g,'window.schoolBoard.saveLunchDuty','lunch renderer -> preload bridge');
must(m,'updateSheetValues(token,UEP_SPREADSHEET_ID,`\'43_야자감독계획\'!${colLetter}${sheetRow}:${colLetter}${sheetRow}`','night sheet update');
must(m,"'42_급식지도계획'",'lunch sheet source');
must(m,"'04_학생연락식별정보'",'contact sheet source');
mustNot(g,'MutationObserver','MutationObserver regression');
