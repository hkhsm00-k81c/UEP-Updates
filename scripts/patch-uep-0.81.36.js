...
// Normalize version first. Match the shipped declaration directly and verify the replacement happened exactly once.
const versionRx=/const\s+APP_VERSION\s*=\s*["'][^"']+["']\s*;/g;
const versionMatches=s.match(versionRx)||[];
if(versionMatches.length!==1)throw new Error('APP_VERSION declaration count='+versionMatches.length);
s=s.replace(versionRx,'const APP_VERSION = "0.81.36";');
if(!s.includes('const APP_VERSION = "0.81.36";'))throw new Error('APP_VERSION replacement failed');
...
`;
const settingsFn=extractFunction(s,'settingsView');if(!settingsFn)throw new Error('settingsView missing');s=s.slice(0,settingsFn.start)+securityHelpers+'\n'+s.slice(settingsFn.start);
let sf=extractFunction(s,'settingsView');let settingsText=sf.text;
...
const subjectCard=String.raw`</div></article>
    ...
    <article class="setting-card role-settings-card">`;
settingsText=settingsText.replace(sensitiveClose,subjectCard);s=s.slice(0,sf.start)+settingsText+s.slice(sf.end);
...
function dashboardOfficialAttendanceLabel(row,fallback='공결'){
  ...
  const direct=String(row?.periodLabel||row?.period||row?.periods||row?.classPeriod||row?.lessonPeriod||'').trim();let period='';const directMatch=direct.match(/(\d{1,2})\s*(?:~|-|–)\s*(\d{1,2})/)||direct.match(/(\d{1,2})/);if(directMatch)period=directMatch[2]?`${directMatch[1]}~${directMatch[2]}교시`:`${directMatch[1]}교시`;
...
`;
let od=extractFunction(s,'openDashboardStudentStatus');if(!od)throw new Error('openDashboardStudentStatus missing');s=s.slice(0,od.start)+attendanceHelper+'\n'+s.slice(od.start);od=extractFunction(s,'openDashboardStudentStatus');let openText=od.text;
...
if(!openText.includes('dashboardOfficialAttendanceLabel'))throw new Error('attendance source-label replacement failed');s=s.slice(0,od.start)+openText+s.slice(od.end);

let be=extractFunction(s,'bindEvents');if(!be)throw new Error('bindEvents missing');let bt=be.text;const openBrace=bt.indexOf('{');const bindInsert=String.raw`
...
if(bt.includes(oldStatus))bt=bt.replace(oldStatus,oldStatus+String.raw`
  $$('[data-dashboard-attendance-student]').forEach(button=>button.onclick=(event)=>{event.preventDefault();event.stopPropagation();attendanceTab='official';attendanceSelectedDate=button.dataset.dashboardAttendanceDate||dateKey(today);attendanceSelectedStudentNo=button.dataset.dashboardAttendanceStudent||'';navigate('attendance');});`);
