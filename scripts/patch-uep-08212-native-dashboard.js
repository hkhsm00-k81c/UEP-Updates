...
const statusDateControl="  const dateControl=cfg.dateSelectable?`<label class=\"dashboard-status-date-picker\"><span>조회 날짜</span><input type=\"date\" value=\"${escapeHtml(basisDate)}\" data-dashboard-status-date></label>`:'';";
must(g.includes(statusDateControl),'dashboard status date control missing');
g=g.replace(statusDateControl,()=>statusDateControl+"\n  const officialLedgerCard=kind==='late'?`<button type=\"button\" class=\"dashboard-official-ledger-card\" data-dashboard-official-ledger><span><small>학교 공결대장</small><b>[운호고] 2026학년도 공결대장</b><em>Google Sheets에서 열기 ↗</em></span></button>`:'';");
const statusBody="${dateControl}<div class=\"history-summary ${studentGroups.length?'warn':''}\">";
must(g.includes(statusBody),'dashboard status body anchor missing');
g=g.replace(statusBody,'${dateControl}${officialLedgerCard}<div class="history-summary ${studentGroups.length?\'warn\':\'\'}">');
const statusDateBinding="  $('#drawerBody [data-dashboard-status-date]')?.addEventListener('change',event=>openDashboardStudentStatus(kind,event.target.value||defaultDate));";
must(g.includes(statusDateBinding),'dashboard status date binding missing');
g=g.replace(statusDateBinding,()=>statusDateBinding+String.raw`
  $('#drawerBody [data-dashboard-official-ledger]')?.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();window.schoolBoard?.openChrome?.('https://docs.google.com/spreadsheets/d/1f6moB3bJJjZcAHv6OPRiLq41InvmzTRuh9ANFZmhNhk/edit');});`);
