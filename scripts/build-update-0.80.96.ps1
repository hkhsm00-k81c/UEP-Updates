...
# Avoid a whole-file regex on the large gyomuon.js. Locate the known selection bundle by literal anchors.
...
# Remove legacy comparison/history composition only.
...
# Replace selectionErrorsForStudent using literal function boundaries rather than regex.
$fnStartNeedle='function selectionErrorsForStudent(student){'
$fnStart=$g.IndexOf($fnStartNeedle,[System.StringComparison]::Ordinal)
if($fnStart -lt 0){throw 'selectionErrorsForStudent start not found'}
$fnEndNeedle='function selectionErrorHistoryForStudent'
$fnEnd=$g.IndexOf($fnEndNeedle,$fnStart,[System.StringComparison]::Ordinal)
if($fnEnd -lt 0){throw 'selectionErrorsForStudent end anchor not found'}
$errorNew=@'
...
'@
$g=$g.Substring(0,$fnStart)+$errorNew+$g.Substring($fnEnd)
...
# The validator already derives from selectedSubjectRows sourced from 06_선택과목이력. Normalize only the requested labels.
...
# Gates for this one-point change.
