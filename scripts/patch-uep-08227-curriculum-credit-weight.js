...
// 18_학교교육과정DB is read by electron/main.cjs together with the other basic-info ranges.
...
// The matrix-to-live-data handoff also lives in electron/main.cjs, not gyomuon.js.
if(!m.includes('data.curriculumDb=uep08210MatrixObjects')){
  const mapAnchor="data.admissionGradeCalcs=uep08210MatrixObjects(matrices['57_내신산정DB']);";
  must(m.includes(mapAnchor),'main 57 mapping anchor not found');
  m=m.replace(mapAnchor,mapAnchor+"\n  data.curriculumDb=uep08210MatrixObjects(matrices['18_학교교육과정DB']);");
  const rawAnchor="data['56_대학입시마스터']=data.universityAdmissions;";
  if(m.includes(rawAnchor)&&!m.includes("data['18_학교교육과정DB']=data.curriculumDb"))m=m.replace(rawAnchor,rawAnchor+"\n  data['18_학교교육과정DB']=data.curriculumDb;");
}
...
const newCredit=`const curriculumRows=Array.isArray(readonlyCache?.curriculumDb)?readonlyCache.curriculumDb:[];
...
must(g.includes('readonlyCache?.curriculumDb'),'renderer curriculum source missing');
must(m.includes("data.curriculumDb=uep08210MatrixObjects(matrices['18_학교교육과정DB'])"),'curriculum cache mapping missing');
