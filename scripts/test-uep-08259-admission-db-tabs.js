...
for(const [name,s] of [['google-data',gd],['main',m]]){
  ...
  must(s.includes('const uep08259AdmissionEntries='),`${name}: admission override entries missing`);
  must(s.includes('readSheetBatch(token,UEP_ADMISSIONS_SPREADSHEET_ID'),`${name}: new admissions DB read missing`);
  must(s.includes("['56_대학입시마스터",`${name}: university master source missing`);
  must(s.includes("['58_권장과목DB",`${name}: recommendations source missing`);
}
