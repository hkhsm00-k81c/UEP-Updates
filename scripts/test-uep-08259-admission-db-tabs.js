...
must(m.includes("'53B_전형유형별대학DB'!A1:S3000"),'main: 53B A:S range missing');
must(m.includes('const UEP_ADMISSIONS_SPREADSHEET_ID = "1BHSdaUbOhp9p_9RB0XGgrfOo4wS_Z8Up0ocSCAMYIIk";'),'main: new admissions spreadsheet id missing');
must(m.includes('const uep08259AdmissionEntries='),'main: admission override entries missing');
must(m.includes('readSheetBatch(token,UEP_ADMISSIONS_SPREADSHEET_ID'),'main: new admissions DB read missing');
must(m.includes("['52_대입기초"),'main: admission basics source missing');
must(m.includes("['54_수능최저DB"),'main: minimum source missing');
must(m.includes("['55_대학입결DB"),'main: result source missing');
must(m.includes("['56_대학입시마스터"),'main: university master source missing');
must(m.includes("['57_내신산정DB"),'main: grade calculation source missing');
must(m.includes("['58_권장과목DB"),'main: recommendations source missing');
