...
function install08181(){
  ...
  function crossScienceChoices08181(data,student,targetTerm){
    if(!data||!student||!targetTerm)return[];const out=[];
    for(const group of (data.subjects||[])){if(String(group.term)!==String(targetTerm)||!isScienceSubject08181(group.subject))continue;const hit=(group.students||[]).find(x=>sameStudent08181(x?.student,student));if(hit)out.push({subject:String(group.subject||''),grade:hit.expectedGrade==null?'-':String(hit.expectedGrade)});}
    return out.sort((a,b)=>a.subject.localeCompare(b.subject,'ko'));
  }
  function formatCrossScience08181(data,student,targetTerm,privacy){if(privacy)return '＊＊＊＊ (＊)';const list=crossScienceChoices08181(data,student,targetTerm);return list.length?list.map(x=>escapeHtml(x.subject)+' ('+escapeHtml(x.grade)+')').join(' · '):'-';}

  const __uepOpenSubjectModal08181=uepOpenSubjectModal08128;
  uepOpenSubjectModal08128=function(key){
    const data=uepSelectionDataset(),selected=(data.subjects||[]).find(x=>x.key===key);__uepOpenSubjectModal08181.apply(this,arguments);
    try{
      if(!selected||!['2-2','3-1'].includes(String(selected.term))||!isScienceSubject08181(selected.subject))return;
      const panel=document.querySelector('.subject-roster-modal');if(!panel)return;ensureStyle08181();
      const targetTerm=String(selected.term)==='2-2'?'3-1':'2-2',roster=panel.querySelector('.selection-roster');if(!roster||roster.classList.contains('roster-seven-columns-08181'))return;
      roster.classList.add('roster-seven-columns-08181');const head=roster.querySelector('.selection-roster-head');if(head){const cell=document.createElement('span');cell.textContent=targetTerm+' 과학선택';head.insertBefore(cell,head.children[1]||null);}
      const rows=[...selected.students].sort(curriculumRosterSort==='class'?((a,b)=>String(a.student.studentNo).localeCompare(String(b.student.studentNo))):((a,b)=>(a.avg??99)-(b.avg??99)||String(a.student.studentNo).localeCompare(String(b.student.studentNo))));const privacy=privacyModeEnabled();
      [...roster.querySelectorAll(':scope > article')].forEach((article,i)=>{const span=document.createElement('span');span.className='roster-cross-science-08181';span.innerHTML=formatCrossScience08181(data,rows[i]?.student,targetTerm,privacy);article.insertBefore(span,article.children[1]||null);});panel.style.cssText+=';width:min(900px,58vw);max-width:900px';
    }catch(error){console.warn('[UEP 0.81.81 roster cross-term]',error);}
  };

  function errorTerms08181(e){const direct=Array.isArray(e?.terms)?e.terms:[],parsed=(String(e?.term||'').match(/[23]-[12]/g)||[]),terms=[...new Set([...direct,...parsed].map(String))];return terms.length?terms:['2-1','2-2','3-1','3-2'];}
  function errorInScope08181(e,scope){if(scope==='all')return true;return errorTerms08181(e).some(t=>t.startsWith(scope+'-'));}
  function errorStudentCounts08181(data){const count=scope=>new Set((data.errors||[]).filter(e=>errorInScope08181(e,scope)).map(e=>studentKey08181(e.student)).filter(Boolean)).size;return {two:count('2'),three:count('3'),all:count('all')};}
  const __uepSelectionDataset08181=uepSelectionDataset;
  uepSelectionDataset=function(){const base=__uepSelectionDataset08181.apply(this,arguments);try{if(curriculumWorkspaceMode!=='students'||curriculumGradeErrorScope08181==='all')return base;return {...base,errors:(base.errors||[]).filter(e=>errorInScope08181(e,curriculumGradeErrorScope08181))};}catch{return base;}};

  function installErrorScopeControls08181(){
    try{
      if(state?.activePage!=='records'||recordMode!=='curriculum'||curriculumWorkspaceMode!=='students')return;const select=document.querySelector('[data-curriculum-error-type]');if(!select)return;ensureStyle08181();curriculumErrorType='all';
      const old=select.closest('label')||select,parent=old.parentElement;if(!parent)return;const counts=errorStudentCounts08181(__uepSelectionDataset08181()),box=document.createElement('div');box.className='uep-error-grade-scope-08181';box.setAttribute('aria-label','오류학생 보기');
      [['2','2학년 오류학생',counts.two],['3','3학년 오류학생',counts.three],['all','2·3학년 오류학생',counts.all]].forEach(([scope,label,count])=>{const b=document.createElement('button');b.type='button';b.dataset.errorGradeScope08181=scope;b.className=curriculumGradeErrorScope08181===scope?'active':'';b.textContent=label+' '+count+'명';box.appendChild(b);});parent.replaceChild(box,old);
  ...
  const __uepRender08181=render;render=function(){const out=__uepRender08181.apply(this,arguments);requestAnimationFrame(()=>setTimeout(installErrorScopeControls08181,0));return out;};

  function closeUpdateNotes08181(){document.getElementById('uepUpdateNotes08181')?.remove();try{localStorage.setItem(UPDATE_SEEN_KEY,UPDATE_VERSION);localStorage.setItem('uep.releaseNotes.seen',UPDATE_VERSION);}catch{}}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(showUpdateNotes08181,900),{once:true});else setTimeout(showUpdateNotes08181,900);setTimeout(installErrorScopeControls08181,300);
}

g+='\n'+START+'\n('+install08181.toString()+')();\n'+END+'\n';
