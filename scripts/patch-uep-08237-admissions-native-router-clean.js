...
// Runtime safety checks: do not allow accidental single-$ selector regression.
must(g.includes("$$('[data-dashboard-admission]').forEach"),'literal $$ admissions selector missing after replacement');
must(!g.includes("$('[data-dashboard-admission]').forEach"),'single-$ admissions selector regression detected');
